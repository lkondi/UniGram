...
//  iOS09
//
//  Created by Lydia Kondylidou on 19.01.18.
...
class NavVC: UINavigationController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UIScrollViewDelegate {
        ...
        if let id = Auth.auth().currentUser?.uid {
            User.info(forUserID: id, completion: {[weak weakSelf = self] (user) in
                DispatchQueue.main.async {
                    ...
                    weakSelf?.profilePicView.image = user.picture
                    weakSelf = nil
                }
            })
        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if self.items.count == 0 {
            ...
            cell.profilePic.image = self.items[indexPath.row].picture
            cell.nameLabel.text = self.items[indexPath.row].userName
